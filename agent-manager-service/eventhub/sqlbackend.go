...
func (b *SQLBackend) pollGatewayWithState(gw *gateway, state GatewayState) error {
	...
	// even when all events were skipped (e.g. only API key events). Without
	// this, lastPolledTime stays zero and every future poll also takes the
	// cold-start path, silencing all subsequent API key events.
