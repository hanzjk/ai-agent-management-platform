...
export const EnvironmentCard = (props: EnvironmentCardProps) => {
  const { environment, orgId, projectId, agentId, actions } = props;
  const theme = useTheme();
        ...
        <CardContent>
          <Box display="flex" flexDirection="row" gap={1} justifyContent="space-between" alignItems="center">
            <Box display="flex" flexDirection="row" gap={1} alignItems="center">
              ...
              <Chip
                icon={<LinkOutlined size={16} color={theme.palette.success.main} />}
                variant="outlined"
                size="small"
                label="Registered"
                color="success"
  ...
  return (
    <Card variant="outlined">
      <CardContent>
          ...
          {currentDeployment.status === DeploymentStatus.ACTIVE && (
            <Box display="flex" flexDirection="row" gap={1} alignItems="center">
              {deployedVersionLabel && (
                <Chip icon={<Tag size={14} />} label={deployedVersionLabel} size="small" variant="outlined" />
              )}
              <Button
                ...
                to={generatePath(
                  absoluteRouteMap.children.org.children.projects.children.agents.children
                    .environment.children.tryOut.path,
                  { orgId, projectId, agentId, envId: environment?.name ?? "" }
                )}
                color="primary"
                size="small"
        ...
        <Divider />
        <Box display="flex" width="100%" justifyContent="center" flexDirection="column" gap={1} pt={2} alignItems="center">
          {currentDeployment.status === DeploymentStatus.INACTIVE && (
            <NoDataFound disableBackground message="Not Deployed" icon={<RocketLaunchOutlined size={32} />} />
          )}
